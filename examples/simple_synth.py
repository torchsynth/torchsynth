...
#
# In this example we'll create a new synth using torchsynth modules. Synth artchitectures can be created using a modular synthesizer paradigm by interconnecting individual modules that each have a unique function. We'll create a simple single oscillator synth with an attack-decay-sustain-release (ADSR) envelope controlling the amplitude in this example. More complicated architectures can be created using the same ideas!
...
#
# All synths in torchsynth derive from `AbstractSynth`, which contains many helpful methods which include funcitonality for keeping track of children `SynthModule`s, creating random patches, and more. See the API documentation for `AbstractSynth` for more information.
#
...
#
# If we instantiate SimpleSynth without passing in a SynthConfig object then it will create one with the default options. We don't need to render a full batch size for this example, so let's use the smallest batch size that will support reproducible output. All the parameters in a synth are randomly assigned values, with reprodcible mode on, we pass a batch_id value into our synth when calling it. The same sounds will always be returned for the same batch_id.

# +
# Create SynthConfig with smallest reproducible batch size -- reprodicible mode is on by default
...
print(
    f"Created {audio.shape[0]} synthesizer sounds that are each {audio.shape[1]} samples long"
)
# -
