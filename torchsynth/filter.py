...
class FIRLowPass(SynthModule0Ddeprecated):
    ...
    ----------
    cutoff : float
            cutoff frequency of low-pass in Hz, must be between 5 and
            half the sampling rate. Defaults to 1000Hz.
    filter_length : int
            The length of the filter in samples. A longer filter will
            result in a steeper filter cutoff. Should be greater than 4.
            Defaults to 512 samples.
    sample_rate: int
            Sampling rate to run processing at.
    ...
    def windowed_sinc(self, cutoff: T, length: T) -> T:
        ...
        ----------
        cutoff: T
                Low-pass cutoff frequency in Hz. Must be between 0 and
                half the sampling rate.
            
        length: T
                Length of the filter impulse response to create.
...
class MovingAverage(SynthModule0Ddeprecated):
    ...
    ----------
    filter_length : int
            Length of filter and number of samples to take average over.
...
class SVF(SynthModule0Ddeprecated):
    ...
    mode : str
            filter type, one of LPF, HPF, BPF, or BSF
    cutoff : float
            cutoff frequency in Hz must be between 5 and
            half the sample rate. Defaults to 1000Hz.
    resonance : float
            filter resonance, or "Quality Factor". Higher
            values cause the filter to resonate more. Must
            be greater than 0.5. Defaults to 0.707.
    mod_depth : float
            Amount of modulation to apply to the cutoff from
    ...
    def _forward(
        ...
        Parameters
        ----------
        audio_in: :obj:`torch.tensor`
                Audio samples to filter
        control_in: :obj:`torch.tensor`
                Control signal used to modulate the filter
